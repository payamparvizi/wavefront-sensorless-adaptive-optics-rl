...
class ReplayBuffer:
    ...
    def __init__(
        ...
        **kwargs: Any,  # otherwise PrioritizedVectorReplayBuffer will cause TypeError
        ...
        self._ep_rew_fiber: Union[float, np.ndarray]
        self.reset()
    ...
    def reset(self, keep_statistics: bool = False) -> None:
        ...
        if not keep_statistics:
            self._ep_rew, self._ep_len, self._ep_idx, self._ep_rew_fiber = 0.0, 0, 0, 0.0
    ...
    def _add_index(self, rew: Union[float, np.ndarray],
                   ...
                   rew_fiber: Union[float, np.ndarray]) -> Tuple[int, Union[float, np.ndarray], int, int]:
        ...
        self._ep_rew += rew
        self._ep_rew_fiber += rew_fiber
        self._ep_len += 1

        if done:
            result = ptr, self._ep_rew, self._ep_len, self._ep_idx, self._ep_rew_fiber
            self._ep_rew, self._ep_len, self._ep_idx, self._ep_rew_fiber = 0.0, 0, self._index, 0.0
            return result
        else:
            return ptr, self._ep_rew * 0.0, 0, self._ep_idx, self._ep_rew_fiber * 0.0
    ...
    def add(
        ...
        try:
            ...
            self._meta[ptr] = batch
        return ptr, ep_rew, ep_len, ep_idx, ep_rew
