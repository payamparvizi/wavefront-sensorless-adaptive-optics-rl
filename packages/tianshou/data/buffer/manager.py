...
class ReplayBufferManager(ReplayBuffer):
    ...
    def add(
        ...
        ptrs, ep_lens, ep_rews, ep_idxs, ep_rews_fiber = [], [], [], [], []
        for batch_idx, buffer_id in enumerate(buffer_ids):
            ptr, ep_rew, ep_len, ep_idx, ep_rew_fiber = self.buffers[buffer_id]._add_index(
                batch.rew[batch_idx], batch.done[batch_idx], batch.info.power[batch_idx]
            )
            ptrs.append(ptr + self._offset[buffer_id])
            ep_lens.append(ep_len)
            ep_rews.append(ep_rew)
            ep_rews_fiber.append(ep_rew_fiber)
        ...
        try:
            ...
            batch.info.power = batch.info.power.astype(float)
            ...
            self._set_batch_for_children()
            self._meta[ptrs] = batch
        return ptrs, np.array(ep_rews), np.array(ep_lens), np.array(ep_idxs), np.array(ep_rews_fiber)
