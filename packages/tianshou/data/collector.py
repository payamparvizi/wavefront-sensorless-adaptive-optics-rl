...
class Collector(object):
    ...
    def collect(
        ...
        while True:
            ...
            ptr, ep_rew, ep_len, ep_idx, ep_rew_fiber = self.buffer.add(
            ...
            if np.any(done):
                ...
                episode_rews_fiber.append(ep_rew_fiber[env_ind_local])
        ...
        if episode_count > 0:
            rews, lens, idxs, rews_fiber = list(
                map(
                    np.concatenate,
                    [episode_rews, episode_lens, episode_start_indices, episode_rews_fiber]
                )
            )
            rew_mean, rew_std = rews.mean(), rews.std()
            rew_mean_fiber, rew_std_fiber = rews_fiber.mean(), rews_fiber.std()
